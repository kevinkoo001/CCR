...
void AsmPrinter::EmitFunctionHeader() {
  ...
  if (MAI->hasFunctionAlignment())
    EmitAlignment(MF->getAlignment(), F); // Koo: Here emits function alignment!
...
bool AsmPrinter::doFinalization(Module &M) {
  ...
  // Koo: store temporary object file name to generate .shuffle.bin 
  ...
  for (unsigned i = 0, e = CPSections.size(); i != e; ++i) {
    for (unsigned j = 0, ee = CPSections[i].CPEs.size(); j != ee; ++j) {
      ...
      if (CurSection != CPSections[i].S) {
        ...
        EmitAlignment(Log2_32(CPSections[i].Alignment)); // Koo: EmitAlignment during EmitConstantPool()
...
void AsmPrinter::EmitJumpTableInfo() {
  ...
  // Koo [Note] Seems that the JT can be emitted in the code section? 
  if (MJTI->getEntryKind() == MachineJumpTableInfo::EK_Inline) {
    errs() << MJTI->getEntryKind() << "***\n";
    MJTI->dump();
    return;
  }
  ...
  for (unsigned JTI = 0, e = JT.size(); JTI != e; ++JTI) {
    ...
    // Koo: Jump Table Information 
    //      Before optimization in BranchFolder::OptimizeBranches() at BranchFolding.cpp
    //      The following output is better than MJTI->dump();
    // errs() << "[JumpTable #" << JTI << "]@[Function #" << MF->getFunctionNumber() << "]\n\t";
    ...
    // Koo [Note]: The entries in a jump table are emitted here!! (but before optimization)
    //             JTI is an index of a JT; Type, EntrySize and index matter.
    //             if -O0 used then all JTs here are valid.
    for (unsigned ii = 0, ee = JTBBs.size(); ii != ee; ++ii) {
      EmitJumpTableEntry(MJTI, JTBBs[ii], JTI);
      // errs() << JTBBs[ii]->getNumber() << " ";
    }
    
    // Koo
    // errs() << "\n\tEntry Kind     : " << MJTI->getEntryKind();
    // errs() << "\n\tEntry Count    : " << JTBBs.size();
    // errs() << "\n\tEntry Size     : " << MJTI->getEntrySize(getDataLayout());
    // errs() << "\n\tEntry Alignment: " << MJTI->getEntryAlignment(getDataLayout()) << "\n\n";
  }
