...
void MCELFStreamer::EmitInstToData(const MCInst &Inst,
  ...
  for (unsigned i = 0, e = Fixups.size(); i != e; ++i) {
    ...
    if (FixupExpr->getKind() == MCExpr::SymbolRef || FixupExpr->getKind() == MCExpr::Binary) {
      ...
      if (SymName.find(JTPrefix) != std::string::npos) {
        ...
        // errs() << "MCELFStreamer::EmitInstToData     [FixupKind = " \
               // << Fixups[i].getKind() << ", MCExpr = " << FixupExpr->getKind() << "]" \
               // << " (" << Fixups[i].getFixupParentID() << ": JT#" \
               // << Fixups[i].getSymbolRefFixupName() << ")\n";
      }
      // Fixups[i].getValue()->dump();
    }
  ...
  // errs() << "MCELFStreamer::EmitInstToData " << Code.size() << "B, STI: " << STI.getByteCtr() << "B\n";
  ...
  // if (ID.length() == 0) {
    // errs() << "MCELFStreamer: " << EmittedBytes << "Fixup: " << numFixups;
    // DF->dump();
    // llvm_unreachable("MCELFStreamer: parentID is empty!!");
  // }
  ...
  // errs() << "[MCELFStreamer] " << EmittedBytes << "\n";
  ...
  // errs() << "MCELFStreamer::EmitInstToData (ID#" << ID << ": " << size << "B, Fixups: " << fixups << "\n";
