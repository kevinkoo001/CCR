...
void X86AsmPrinter::LowerSTATEPOINT(const MachineInstr &MI,
  ...
  if (unsigned PatchBytes = SOpers.getNumPatchBytes()) {
    EmitNops(*OutStreamer, PatchBytes, Subtarget->is64Bit(), getSubtargetInfo()); 
  } else {
...
void X86AsmPrinter::LowerPATCHABLE_OP(const MachineInstr &MI,
  ...
  if (Code.size() < MinSize) {
    if (MinSize == 2 && Opcode == X86::PUSH64r) {
      ...
      unsigned NopSize = EmitNop(*OutStreamer, MinSize, Subtarget->is64Bit(), getSubtargetInfo()); 
...
#define CASE_ALL_MOV_RM()               \
  ...
  if (MBB) {
    ...
    errs() << "[X86MCInstLower] No parent for the following instruction!\n\t";
    TmpInst.dump();
  }
